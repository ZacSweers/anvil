...
import com.squareup.anvil.compiler.internal.ksp.anySymbolsWithAnnotations
import com.squareup.anvil.compiler.internal.ksp.argumentOfTypeAt
import com.squareup.anvil.compiler.internal.ksp.atLeastOneAnnotation
import com.squareup.anvil.compiler.internal.ksp.classId
import com.squareup.anvil.compiler.internal.ksp.classNameArgumentAt
import com.squareup.anvil.compiler.internal.ksp.classNameArrayArgumentAt
import com.squareup.anvil.compiler.internal.ksp.contextualToClassName
import com.squareup.anvil.compiler.internal.ksp.contextualToTypeName
...
import com.squareup.anvil.compiler.internal.ksp.findAll
...
import com.squareup.anvil.compiler.internal.ksp.includes
...
import com.squareup.anvil.compiler.internal.ksp.reportableReturnTypeNode
...
import com.squareup.anvil.compiler.internal.ksp.scope
import com.squareup.anvil.compiler.internal.ksp.scopeClassName
import com.squareup.anvil.compiler.internal.ksp.superTypesExcludingAny
import com.squareup.anvil.compiler.internal.ksp.toFunSpec
import com.squareup.anvil.compiler.internal.ksp.toPropertySpec
...
internal class KspContributionMerger(
  ...
  private fun processClass(
    ...
    val (contributedInterfaces, directMergeSubcomponents) = trace(
    ...
    trace("Generating ${generatedComponentClassName.simpleName}") {
      generateMergedClass(
        resolver = resolver,
  ...
  private fun findContributedInterfaces(
    ...
    val contributedInterfaces = trace("Finding contributed interfaces") {
    ...
    val directMergeSubcomponents = trace("Finding directly merged subcomponents") {
      findMergedSubcomponentInterfaces(
        resolver,
        mergeAnnotatedClass,
        contributedInterfaces,
      )
  ...
  private fun generateDaggerAnnotation(
        ...
        if (includedModule.isAnnotationPresent<Module>()) {
          ...
          moduleClassName.mergedClassName().also { mergedModuleClass ->
            if (includedModule.toClassName() in contributedModulesInRound) {
              ...
              resolver.getClassDeclarationByName(mergedModuleClass)?.let {
    ...
    val allContributesAnnotations: List<KSAnnotation> = annotations
      ...
      .flatMap { annotation ->
        ...
        classScanner
          ...
          .map {
            resolver.requireClassDeclaration(it.className, node = null)
          }
          .plus(
    ...
    val excludedModules = trace("Excluding modules") {
      annotations
    ...
    val replacedModules = trace("Replaced modules") {
      allContributesAnnotations
    ...
    val bindings = trace("Contributed bindings") {
      bindingModuleContributesAnnotations
    ...
    if (predefinedModules.isNotEmpty()) {
      trace("Checking conflicting includes and excludes") {
    ...
    val contributedSubcomponentModules = trace("Finding contributed subcomponent modules") {
    ...
    val contributedModules: List<ClassName> = trace("Computing final modules list") {
      contributesAnnotations
        ...
        .plus(
          bindings.bindings.values.flatMap(Map<BindingKey<*>, List<ContributedBinding<*>>>::values)
            ...
            .map { bindingModule ->
              resolver.requireClassDeclaration(bindingModule, node = null)
                .also {
                  originatingDeclarations += it
                }
                .toClassName()
  ...
  private fun contributedInterfaces(
      ...
      mergeAnnotations
        .flatMap { annotation ->
          ...
          classScanner
            ...
            .flatMap {
              val declaration = resolver.requireClassDeclaration(
                it.className,
                node = { annotation },
              )
    ...
    val replacedClasses = trace("Replaced classes") {
      contributesAnnotations
    ...
    val excludedClasses = trace("Excluded classes") {
      mergeAnnotations
        .asSequence()
        .flatMap(KSAnnotation::exclude)
    ...
    if (excludedClasses.isNotEmpty()) {
      trace("Checking excluded classes for intersection with replaced classes") {
    ...
    val supertypesToAdd: List<ClassName> = trace("Computing final supertypes") {
      contributesAnnotations
        .asSequence()
        .map { it.declaringClass.toClassName() }
        .plus(
        ...
        .plus(
          findContributedSubcomponentParentInterfaces(
            ...
            resolver.getClassDeclarationByName(contributedInterface)?.let {
              originatingDeclarations += it
            }
          },
        )
    ...
    val generatedComponent = TypeSpec.interfaceBuilder(
      ...
      .apply {
        ...
        if (contributedSubcomponentData != null) {
          ...
          if (contributor != null) {
            val parentClass = resolver.requireClassDeclaration(
              contributor,
              node = { mergeAnnotatedClass },
            )
    ...
    val contributedCallables =
      contributedInterfaces.asSequence()
        .mapNotNull { contributedInterface ->
          resolver.getClassDeclarationByName(contributedInterface)
        }
...
private data class ContributedSubcomponentData(
  ...
  fun resolveCreatorDeclaration(resolver: Resolver): KSClassDeclaration? {
    ...
    return resolver.getClassDeclarationByName(creator)
  }
